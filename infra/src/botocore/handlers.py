...
def generate_idempotent_uuid(params, model, **kwargs):
    for name in model.idempotent_members:
        if name not in params:
            ...
            logger.debug(f"injecting idempotency token ({params[name]}) into param '{name}'.")
...
def _handle_200_error(operation_model, response_dict, **kwargs):
    ...
    if _looks_like_special_case_error(response_dict["status_code"], response_dict["body"]):
        ...
        logger.debug(f"Error found for response with 200 status code: {response_dict['body']}.")
