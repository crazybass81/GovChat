...
class EndpointResolver(BaseEndpointResolver):
    ...
    def _endpoint_for_partition(
        ...
        if use_dualstack_endpoint and partition_name in self._UNSUPPORTED_DUALSTACK_PARTITIONS:
            error_msg = (
                f"Dualstack endpoints are currently not supported for {partition_name} partition"
            )
    ...
    def _resolve(
        ...
        if endpoint_data.get("deprecated"):
            LOG.warning(f"Client is configured with the deprecated endpoint: {endpoint_name}")
