...
class Lexer:
        ...
        while self._current is not None:
            if self._current in self.SIMPLE_TOKENS:
                ...
                if len(buff) > 1:
                    ...
                    raise LexerError(
                        lexer_position=start, lexer_value=buff, message="Unknown token '%s'" % buff
                    )
            elif self._current == '"':
                ...
                if self._next() == "=":
                    ...
                    raise LexerError(
                        lexer_position=position, lexer_value="=", message="Unknown token '='"
                    )
            else:
                raise LexerError(
    ...
    def _consume_literal(self):
        ...
        return {"type": "literal", "value": parsed_json, "start": start, "end": token_len}
