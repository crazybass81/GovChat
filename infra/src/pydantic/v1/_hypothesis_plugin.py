...
_color_regexes = (
    ...
    .replace(
        pydantic.color._r_255, r"(?:((?:\d|\d\d|[01]\d\d|2[0-4]\d|25[0-4])(?:\.\d+)?|255(?:\.0+)?))"
    )
)
st.register_type_strategy(
...
# FutureDate, PastDate
st.register_type_strategy(
    pydantic.FutureDate, st.dates(min_value=datetime.date.today() + datetime.timedelta(days=1))
)
st.register_type_strategy(
    pydantic.PastDate, st.dates(max_value=datetime.date.today() - datetime.timedelta(days=1))
)
...
def resolve_confloat(cls):  # type: ignore[no-untyped-def]
    ...
    if cls.multiple_of is None:
        return st.floats(
            min_value, max_value, exclude_min=exclude_min, exclude_max=exclude_max, allow_nan=False
        )
