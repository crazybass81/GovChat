        ...
        return "{name}({kwargs})".format(
            ...
            kwargs=", ".join(
                ...
                for key, value in sorted(
                    attr.asdict(self.config, recurse=True).items(), key=lambda x: x[0]
                )
            ),
        )

    def master_keys_for_encryption(
        self, encryption_context, plaintext_rostream, plaintext_length=None
    ):
    ...
    def decrypt_data_key(self, encrypted_data_key, algorithm, encryption_context):
        ...
        for member in [self] + self._members:
            ...
            if member.provider_id == encrypted_data_key.key_provider.provider_id:
                _LOGGER.debug(
                    "attempting to locate master key from key provider: %s", member.provider_id
                )
                if isinstance(member, MasterKey):
                    ...
                    try:
                        _LOGGER.debug(
                            "attempting to add master key: %s",
...
class MasterKeyConfig:
    ...
    def __attrs_post_init__(self):
        ...
        if not hasattr(self, "provider_id"):
            raise TypeError(
                'Instances of MasterKeyConfig must have a "provider_id" attribute defined.'
            )
...
class MasterKey(MasterKeyProvider):
        ...
        if instance.config.provider_id is not None:
            ...
            if instance.provider_id is None:
                ...
                raise ConfigMismatchError(
                    f"Config provider_id does not match MasterKey provider_id: {instance.config.provider_id} != {instance.provider_id}"
                )
    ...
    def _new_master_key(self, key_id):
        ...
        if key_id != self.key_id:
            raise InvalidKeyIdError(
                f"MasterKeys can only provide themselves.  Requested {key_id} but only {self.key_id} is available"
            )
        return self
    ...
    def _key_check(self, data_key):
        ...
        if not self.owns_data_key(data_key):
            raise IncorrectMasterKeyError(
                f"Provided data key provider {data_key.key_provider} does not match Master Key provider {self.key_provider}"
            )
    ...
    def encrypt_data_key(self, data_key, algorithm, encryption_context):
        ...
        return self._encrypt_data_key(
            data_key=data_key, algorithm=algorithm, encryption_context=encryption_context
        )
