    ...
    try:
        ...
        if default_region is None:
            raise UnknownRegionError(
                f"No default region found and no region determinable from key id: {key_id}"
            )
...
class KMSMasterKey(MasterKey):
    ...
    def _generate_data_key(self, algorithm, encryption_context=None):
        ...
        try:
            ...
            error_message = f"Master Key {self._key_id} unable to generate data key"
        ...
        try:
            ...
            error_message = f"Retrieved an unexpected KeyID in response from KMS: {key_id}"
            _LOGGER.exception(error_message)
            raise GenerateKeyError(error_message)
    ...
    def _encrypt_data_key(self, data_key, algorithm, encryption_context=None):
        ...
        try:
            ...
            error_message = f"Master Key {self._key_id} unable to encrypt data key"
            _LOGGER.exception(error_message)
    ...
    def _decrypt_data_key(self, encrypted_data_key, algorithm, encryption_context=None):
        ...
        if not edk_arn.resource_type == "key":
            error_message = f"AWS KMS Provider EDK contains unexpected key_id: {edk_key_id}"
            _LOGGER.exception(error_message)
        ...
        try:
            ...
            if returned_key_id != self._key_id:
                error_message = (
                    f"AWS KMS returned unexpected key_id {returned_key_id} (expected {self._key_id})"
                )
                _LOGGER.exception(error_message)
                raise DecryptKeyError(error_message)
            ...
            if len(plaintext) != algorithm.data_key_len:
                error_message = f"Plaintext length ({len(plaintext)}) does not match algorithm's expected length ({algorithm.data_key_len})"
                raise DecryptKeyError(error_message)

        except (ClientError, KeyError):
            error_message = f"Master Key {self._key_id} unable to decrypt data key"
            _LOGGER.exception(error_message)
            raise DecryptKeyError(error_message)
        return DataKey(
    ...
    def _validate_allowed_to_decrypt(self, edk_key_id):
        ...
        if edk_key_id != self._key_id:
            raise DecryptKeyError(
                f"Cannot decrypt EDK wrapped by key_id={edk_key_id}, because it does not match this "
                f"provider's key_id={self._key_id}"
            )
...
class MRKAwareKMSMasterKey(KMSMasterKey):
    ...
    def owns_data_key(self, data_key):
        ...
        # //# "true".
        if (
            data_key.key_provider.provider_id == self.key_provider.provider_id
            and _check_mrk_arns_equal(
...
class KMSMasterKeyProviderConfig(MasterKeyProviderConfig):
    ...
    discovery_filter = attr.ib(
        hash=True,
        default=None,
        validator=attr.validators.optional(attr.validators.instance_of(DiscoveryFilter)),
    )
    ...
    def add_regional_client(self, region_name):
        ...
        if region_name not in self._regional_clients:
            ...
            client = session.client(
                "kms", region_name=region_name, config=self._user_agent_adding_config
            )
        ...
        if self.config.discovery_filter:
            ...
            if (
                ...
                raise MasterKeyProviderError(
                    f"Key {key_id} not allowed by this Master Key Provider"
                )
...
class MRKAwareStrictAwsKmsMasterKeyProvider(StrictAwsKmsMasterKeyProvider):
    ...
    def validate_unique_mrks(self):
        ...
        if len(duplicate_ids) > 0:
            raise ConfigMismatchError(
                "Configured key ids must be unique. Found related MRKs: {keys}".format(
                    keys=", ".join(duplicate_ids)
                )
            )
...
class DiscoveryAwsKmsMasterKeyProvider(BaseKMSMasterKeyProvider):
    ...
    def validate_config(self):
        ...
        if self.config.discovery_filter:
            if (
                not self.config.discovery_filter.account_ids
                or not self.config.discovery_filter.partition
            ):
                raise ConfigMismatchError(
