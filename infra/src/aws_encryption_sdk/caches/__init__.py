...
def build_encryption_materials_cache_key(partition, request):
    ...
    _ec_hash = _encryption_context_hash(
        hasher=hasher.copy(), encryption_context=request.encryption_context
    )
...
def build_decryption_materials_cache_key(partition, request):
    ...
    _edks_hash = _encrypted_data_keys_hash(
        hasher=hasher.copy(), encrypted_data_keys=request.encrypted_data_keys
    )
    _ec_hash = _encryption_context_hash(
        hasher=hasher.copy(), encryption_context=request.encryption_context
    )
...
@attr.s(hash=False)
class CryptoMaterialsCacheEntryHints:
    ...
    lifetime = attr.ib(
        default=None, validator=attr.validators.optional(attr.validators.instance_of(float))
    )


@attr.s(hash=False)
class CryptoMaterialsCacheEntry:
    ...
    value = attr.ib(
        validator=attr.validators.instance_of((EncryptionMaterials, DecryptionMaterials))
    )
    hints = attr.ib(
        default=attr.Factory(CryptoMaterialsCacheEntryHints),
        validator=attr.validators.optional(
            attr.validators.instance_of(CryptoMaterialsCacheEntryHints)
        ),
    )
    ...
    def __setattr__(self, name, value):
        ...
        if hasattr(self, "_init_completed"):
            raise NotSupportedError(
                "Attributes may not be set on CryptoMaterialsCacheEntry objects"
            )
    ...
    def _update_with_message_bytes_encrypted(self, bytes_encrypted):
        ...
        with self._lock:
            super(CryptoMaterialsCacheEntry, self).__setattr__(
                "messages_encrypted", self.messages_encrypted + 1
            )
            super(CryptoMaterialsCacheEntry, self).__setattr__(
