...
class LocalCryptoMaterialsCache(CryptoMaterialsCache):
    ...
    def __setattr__(self, name, value):
        ...
        if hasattr(self, "_init_completed") and name == "capacity":
            raise NotSupportedError(
                "capacity may not be modified on LocalCryptoMaterialsCache instances"
            )
    ...
    def put_encryption_materials(
        self, cache_key, encryption_materials, plaintext_length, entry_hints=None
    ):
        ...
        entry = CryptoMaterialsCacheEntry(
            cache_key=cache_key, value=encryption_materials, hints=entry_hints
        )
    ...
    def get_encryption_materials(self, cache_key, plaintext_length):
        ...
        """
        _LOGGER.debug(
            "Looking in cache for encryption materials to encrypt %d bytes.", plaintext_length
        )
        with self._cache_lock:
