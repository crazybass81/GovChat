...
from aws_encryption_sdk.internal.crypto.data_keys import (
    calculate_commitment_key,
...
from aws_encryption_sdk.internal.utils.signature import (
    SignaturePolicy,
    validate_signature_policy_on_decrypt,
)
...
class _ClientConfig:  # pylint: disable=too-many-instance-attributes
...
class EncryptorConfig(_ClientConfig):
    ...
    frame_length = attr.ib(
        hash=True, default=FRAME_LENGTH, validator=attr.validators.instance_of(six.integer_types)
    )
...
class StreamEncryptor(_EncryptionStream):  # pylint: disable=too-many-instance-attributes
    ...
    def _read_bytes_to_framed_body(self, b):
        ...
        _LOGGER.debug(
            "%d bytes requested; reading %d bytes after normalizing to frame length", _b, b
        )
...
class StreamDecryptor(_EncryptionStream):  # pylint: disable=too-many-instance-attributes
    ...
    def _read_header(self):
        ...
        """
        header, raw_header = deserialize_header(
            self.source_stream, self.config.max_encrypted_data_keys
        )
        ...
        if (
            ...
            raise CustomMaximumValueExceeded(
                f"Frame Size in header found larger than custom value: {header.frame_length:d} > {self.config.max_body_length:d}"
            )
    ...
    def _prep_non_framed(self):
        ...
        if (
            ...
            raise CustomMaximumValueExceeded(
                f"Non-framed message content length found larger than custom value: {self.body_length:d} > {self.config.max_body_length:d}"
            )
