...
class CachingCryptoMaterialsManager(CryptoMaterialsManager):
    ...
    max_bytes_encrypted = attr.ib(
        default=MAX_BYTES_PER_KEY, validator=attr.validators.instance_of(six.integer_types)
    )
    ...
    def __attrs_post_init__(self):
        ...
        if self.max_messages_encrypted > MAX_MESSAGES_PER_KEY:
            raise ValueError(f"max_messages_encrypted cannot exceed {MAX_MESSAGES_PER_KEY}")

        if self.max_bytes_encrypted > MAX_BYTES_PER_KEY:
            raise ValueError(f"max_bytes_encrypted cannot exceed {MAX_BYTES_PER_KEY}")
        ...
        if self.backing_materials_manager is None:
            if self.master_key_provider is None:
                raise TypeError(
                    "Either backing_materials_manager or master_key_provider must be defined"
                )
    ...
    def _should_cache_encryption_request(self, request):
        ...
        if request.plaintext_length is None:
            _LOGGER.debug(
                "Encryption materials request not cached because plaintext length is unknown"
            )
            return False

        if request.algorithm is not None and not request.algorithm.safe_to_cache():
            _LOGGER.debug(
                "Encryption materials request not cached because algorithm suite is not safe to cache"
            )
            return False
    ...
    def get_encryption_materials(self, request):
        ...
        if (
            not new_result.algorithm.safe_to_cache()
    ...
    def decrypt_materials(self, request):
        ...
        """
        cache_key = build_decryption_materials_cache_key(
            partition=self.partition_name, request=request
        )
