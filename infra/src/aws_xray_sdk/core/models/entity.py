...
class Entity:
    ...
    def __init__(self, name, entity_id=None):
        ...
        if self.name != name:
            log.warning(f"Removing Segment/Subsugment Name invalid characters from {name}.")
    ...
    def add_exception(self, exception, stack, remote=False):
        ...
        if hasattr(exception, "_recorded"):
            self.cause = exception._cause_id
            return

        if not isinstance(self.cause, dict):
            log.warning(
                f"The current cause object is not a dict but an id: {self.cause}. Resetting the cause and recording the "
                "current exception"
            )
            self.cause = {}
