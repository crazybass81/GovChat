...
class LambdaContext(Context):
    ...
    def put_subsegment(self, subsegment):
        ...
        if not self._is_subsegment(current_entity) and (
            getattr(current_entity, "initializing", None)
            or isinstance(current_entity, DummySegment)
        ):
    ...
    def set_trace_entity(self, trace_entity):
        ...
        self._local.entities = [trace_entity]
