...
def _is_valid_import(module):
    ...
    is_module = os.path.isdir(realpath) and (
        os.path.isfile(f"{module}/__init__.py")
        or os.path.isfile(f"{module}/__init__.pyc")
    )
    is_file = not is_module and (
        os.path.isfile(f"{module}.py") or os.path.isfile(f"{module}.pyc")
    )
...
def patch(modules_to_patch, raise_errors=True, ignore_module_patterns=None):
    ...
    if unsupported_modules:
        raise Exception(
            "modules %s are currently not supported for patching" % ", ".join(unsupported_modules)
        )
...
def _patch_func(parent, func_name, func, modifier=lambda x: x):
    ...
    if func_name.startswith("__") and func_name.endswith("__"):
        capture_name = f"{parent.__name__}.{capture_name}"
    ...
    for member_name, member in inspect.getmembers(cls, inspect.ismethod):
        if member.__module__ == module.__name__:
            ...
            if is_classmethod(member):
                ...
                log.warning(
                    "Cannot automatically patch classmethod %s.%s, please apply decorator manually",
                    cls.__name__,
                    member_name,
                )
            else:
...
def _external_module_patch(module, ignore_module_patterns):
    if module.startswith("."):
        raise Exception(f"relative packages not supported for patching: {module}")
