    ...
    for each in instance:
        if contains_validator.is_valid(each):
            ...
            if matches > max_contains:
                yield ValidationError(
                    f"Too many items match the given schema (expected at most {max_contains})",
...
def exclusiveMinimum(validator, minimum, instance, schema):
    ...
    if instance <= minimum:
        yield ValidationError(
            f"{instance!r} is less than or equal to the minimum of {minimum!r}",
        )
...
def exclusiveMaximum(validator, maximum, instance, schema):
    ...
    if instance >= maximum:
        yield ValidationError(
            f"{instance!r} is greater than or equal to the maximum of {maximum!r}",
        )
