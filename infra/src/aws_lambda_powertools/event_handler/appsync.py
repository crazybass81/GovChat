...
from aws_lambda_powertools.event_handler.graphql_appsync.exceptions import (
...
class AppSyncResolver(Router):
    ...
    def _call_single_resolver(self, event: dict, data_model: type[AppSyncResolverEvent]) -> Any:
        ...
        resolver = self._resolver_registry.find_resolver(
            self.current_event.type_name, self.current_event.field_name
        )
        if not resolver:
            raise ValueError(
                f"No resolver found for '{self.current_event.type_name}.{self.current_event.field_name}'"
            )
    ...
    def _call_sync_batch_resolver(
        ...
        for idx, event in enumerate(self.current_batch_event):
            try:
                ...
                logger.debug(
                    f"Failed to process event number {idx} from field '{event.info.field_name}'"
                )
                results.append(None)
    ...
    def _call_batch_resolver(
        self, event: list[dict], data_model: type[AppSyncResolverEvent]
    ) -> list[Any]:
