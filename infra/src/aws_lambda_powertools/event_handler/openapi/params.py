...
def analyze_param(
    *,
    ...
    field_info, type_annotation = get_field_info_and_type_annotation(
        annotation, value, is_path_param
    )
    ...
    if isinstance(value, FieldInfo):
        if field_info is not None:
            raise AssertionError(
                "Cannot use a FieldInfo as a parameter annotation and pass a FieldInfo as a value"
            )
        field_info = value
...
def get_field_info_and_type_annotation(
    ...
    if annotation is not inspect.Signature.empty:
        ...
        if get_origin(annotation) is Annotated:
            field_info, type_annotation = get_field_info_annotated_type(
                annotation, value, is_path_param
            )
...
def get_field_info_annotated_type(
    annotation, value, is_path_param: bool
) -> tuple[FieldInfo | None, Any]:
    """
