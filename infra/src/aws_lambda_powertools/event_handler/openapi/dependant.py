...
def is_body_param(*, param_field: ModelField, is_path_param: bool) -> bool:
    ...
    if is_path_param:
        ...
        return False
    elif isinstance(param_field.field_info, (Query, Header)) and is_scalar_sequence_field(
        param_field
    ):
        return False
    else:
        if not isinstance(param_field.field_info, Body):
            raise AssertionError(
                f"Param: {param_field.name} can only be a request body, use Body()"
            )
        return True
    ...
    *,
    ...
    if any(isinstance(f.field_info, _File) for f in flat_dependant.body_params):
        ...
        body_param_media_types = [
            ...
            if isinstance(f.field_info, Body)
        ]
