...
class Route:
    ...
    def _build_middleware_stack(self, router_middlewares: list[Callable[..., Any]]) -> None:
        ...
        for handler in reversed(all_middlewares):
            self._middleware_stack = MiddlewareFrame(
                current_middleware=handler, next_middleware=self._middleware_stack
            )
    ...
    def dependant(self) -> Dependant:
        if self._dependant is None:
            ...
            self._dependant = get_dependant(
                path=self.openapi_path, call=self.func, responses=self.responses
            )
                ...
                # Case 2: there is a 'content' key
                    ...
                    for content_type, payload in response["content"].items():
                        ...
                        if "model" in payload:
                            ...
                            return_field = next(
                                filter(
                                    lambda model: model.type_
                                    is cast(OpenAPIResponseContentModel, payload)["model"],
                                    self.dependant.response_extra_models,
                                ),
                            )
                            if not return_field:
                                raise AssertionError(
                                    "Model declared in custom responses was not found"
                                )
    ...
    def _openapi_operation_metadata(self, operation_ids: set[str]) -> dict[str, Any]:
        ...
        if self.operation_id in operation_ids:
            message = (
                f"Duplicate Operation ID {self.operation_id} for function {self.func.__name__}"
            )
...
def _registered_api_adapter(
    app: ApiGatewayResolver, next_middleware: Callable[..., Any]
) -> dict | tuple | Response:
    """
...
class ApiGatewayResolver(BaseRouter):
    ...
    def _get_openapi_security(
        ...
        if not _validate_openapi_security_parameters(
            security=security, security_schemes=security_schemes
        ):
            raise SchemaValidationError(
                "Security configuration was not found in security_schemas or security_schema was not defined. "
                "See: https://docs.powertools.aws.dev/lambda/python/latest/core/event_handler/api_gateway/#security-schemes",
    ...
    def _handle_not_found(self, method: str, path: str) -> ResponseBuilder:
        ...
        def not_found_handler():
            ...
            if self._cors and method == "OPTIONS":
                ...
                _headers["Access-Control-Allow-Methods"] = CORSConfig.build_allow_methods(
                    self._cors_methods
                )
    ...
    def _call_exception_handler(self, exp: Exception, route: Route) -> ResponseBuilder | None:
        ...
        if handler:
            try:
                return self._response_builder_class(
                    response=handler(exp), serializer=self._serializer, route=route
                )
    ...
    def route(
        ...
        def register_route(func: Callable):
            ...
            fronzen_openapi_extensions = (
                _FrozenDict(openapi_extensions) if openapi_extensions else None
            )
...
class APIGatewayRestResolver(ApiGatewayResolver):
    ...
    def _get_base_path(self) -> str:
        ...
        if (
            stage
            and stage != "$default"
            and self.current_event.request_context.path.startswith(f"/{stage}")
        ):
            return f"/{stage}"
        return ""
...
class APIGatewayHttpResolver(ApiGatewayResolver):
    ...
    def _get_base_path(self) -> str:
        ...
        if (
            stage
            and stage != "$default"
            and self.current_event.request_context.http.path.startswith(f"/{stage}")
        ):
            return f"/{stage}"
        return ""
...
class ALBResolver(ApiGatewayResolver):
    ...
    def __init__(
        ...
        super().__init__(
            ProxyEventType.ALBEvent, cors, debug, serializer, strip_prefixes, enable_validation
        )
