...
class APIGatewayProxyEvent(BaseProxyEvent):
    ...
    def multi_value_query_string_parameters(self) -> dict[str, list[str]]:
        return (
            self.get("multiValueQueryStringParameters") or {}
        )  # key might exist but can be `null`
...
class RequestContextV2AuthorizerIam(DictWrapper):
    ...
    def _cognito_identity(self) -> dict:
        return (
            self.get("cognitoIdentity") or {}
        )  # not available in FunctionURL; key might exist but can be `null`
    ...
    def cognito_identity_pool_id(self) -> str:
        ...
        Available only if the request was signed with Amazon Cognito credentials."""
        return (
            self._cognito_identity().get("identityPoolId") or ""
        )  # key might exist but can be `null`
