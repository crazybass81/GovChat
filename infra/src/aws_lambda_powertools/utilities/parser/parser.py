...
def event_parser(
    ...
    **Lambda handler decorator to parse & validate event - using built-in envelope**
    ...
    try:
        ...
        raise InvalidModelTypeError(
            f"Error: {str(exc)}. Please ensure the type you're trying to parse into is correct"
        )
...
@overload
def parse(
    event: dict[str, Any], model: type[T], envelope: type[Envelope]
) -> T: ...  # pragma: no cover
...
def parse(event: dict[str, Any], model: type[T], envelope: type[Envelope] | None = None):
    ...
    **Lambda handler decorator to parse & validate event - using built-in envelope**
    ...
    # when we pass a data structure that does not match the model (trying to parse a true/false/etc into a model)
        ...
        raise InvalidModelTypeError(
            f"The event supplied is unable to be validated into {type(model)}"
        ) from exc
