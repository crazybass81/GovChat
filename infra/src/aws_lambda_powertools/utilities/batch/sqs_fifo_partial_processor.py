...
class SqsFifoPartialProcessor(BatchProcessor):
    ...
    group_circuit_breaker_exc = (
        SQSFifoMessageGroupCircuitBreakerError,
        SQSFifoMessageGroupCircuitBreakerError(
            "A previous record from this message group failed processing"
        ),
        None,
    )
    ...
    def _process_record(self, record):
        ...
        fail_group_id = (
            self._skip_group_on_error and self._current_group_id in self._failed_group_ids
        )
        if fail_entire_batch or fail_group_id:
            return self.failure_handler(
                ...
                exception=self.group_circuit_breaker_exc
                if self._skip_group_on_error
                else self.circuit_breaker_exc,
            )
