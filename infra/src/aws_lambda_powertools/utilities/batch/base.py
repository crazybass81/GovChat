...
class BasePartialProcessor(ABC):
    ...
    def __call__(
        self, records: list[dict], handler: Callable, lambda_context: LambdaContext | None = None
    ):
        """
...
class BasePartialBatchProcessor(BasePartialProcessor):  # noqa
    ...
    ) -> EventSourceDataClassTypes: ...  # pragma: no cover

    def _to_batch_type(
        self, record: dict, event_type: EventType, model: BatchTypeModels | None = None
    ):
