...
class RedisConnection:
    ...
    def _init_client(self) -> RedisClientProtocol:
        ...
        try:
            ...
            raise IdempotencyPersistenceConnectionError(
                "Could not to connect to Redis", exc
            ) from exc
...
class RedisCachePersistenceLayer(BasePersistenceLayer):
    ...
    def _put_in_progress_record(self, data_record: DataRecord) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Putting record on Redis for idempotency key: {data_record.idempotency_key}"
            )
            encoded_item = self._json_serializer(item["mapping"])
            ttl = self._get_expiry_second(expiry_timestamp=data_record.expiry_timestamp)

            redis_response = self.client.set(
                name=data_record.idempotency_key, value=encoded_item, ex=ttl, nx=True
            )
            ...
            if (
                idempotency_record.status == STATUS_CONSTANTS["COMPLETED"]
                and not idempotency_record.is_expired
            ):
    ...
    def _acquire_lock(self, name: str):
        ...
        try:
            acquired = self.client.set(
                name=f"{name}:lock", value="True", ex=self._orphan_lock_timeout, nx=True
            )
