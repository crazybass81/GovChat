...
from aws_lambda_powertools.metrics.provider.cloudwatch_emf.metric_properties import (
    MetricResolution,
    MetricUnit,
)
    ...
    **Use `aws_lambda_powertools.metrics.metrics.Metrics` or
    ...
    def serialize_metric_set(
        ...
        **Serialize metrics into EMF format**
        ...
        logger.debug(
            {"details": "Serializing metrics", "metrics": metrics, "dimensions": dimensions}
        )
        ...
        return {
            "_aws": {
                ...
                or int(datetime.datetime.now().timestamp() * 1000),  # epoch
                "CloudWatchMetrics": [
                    {
    ...
    def _add_cold_start_metric(self, context: Any) -> None:
        ...
        if is_cold_start:
            ...
            with single_metric(
                name="ColdStart", unit=MetricUnit.Count, value=1, namespace=self.namespace
            ) as metric:
