...
class DatadogProvider(BaseProvider):
    ...
    **Use `aws_lambda_powertools.DatadogMetrics` to create and metrics to Datadog.**
    ...
    def __init__(
        ...
        self.flush_to_log = resolve_env_var_choice(
            choice=flush_to_log, env=os.getenv(constants.DATADOG_FLUSH_TO_LOG)
        )
            ...
            output_list.append(
                {
                    ...
                    "v": single_metric["v"],
                    "e": single_metric["e"],
                    "t": self._serialize_datadog_tags(
                        metric_tags=single_metric["t"], default_tags=self.default_tags
                    ),
                },
            )
    ...
    @staticmethod
    def _serialize_datadog_tags(
        metric_tags: dict[str, Any], default_tags: dict[str, Any]
    ) -> list[str]:
        """
    ...
    def _validate_datadog_metric_name(metric_name: str) -> bool:
        ...
        if (
            not metric_name[0].isalpha()
            or len(metric_name) > 200
            or not METRIC_NAME_REGEX.match(metric_name)
        ):
            return False
