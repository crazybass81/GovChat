...
class Logger:
    ...
    **Logging to a file instead of standard output for testing**
    ...
    def structure_logs(
        self, append: bool = False, formatter_options: dict | None = None, **keys
    ) -> None:
        ...
        if not is_logger_preconfigured:
            formatter = self.logger_formatter or LambdaPowertoolsFormatter(
                **formatter_options, **log_keys
            )
            ...
            custom_keys = {
                k: v for k, v in log_keys.items() if k not in RESERVED_FORMATTER_CUSTOM_KEYS
            }
    ...
    def _get_aws_lambda_log_level(self) -> str | None:
        ...
        return constants.LAMBDA_ADVANCED_LOGGING_LEVELS.get(
            os.getenv(constants.LAMBDA_LOG_LEVEL_ENV)
        )
