...
class AWS4Auth(AuthBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.refreshable_credentials:
            ...
            if not self.service:
                raise TypeError(
                    "service must be provided as keyword argument when using refreshable_credentials"
                )
            self.region = kwargs.get("region", None)
            if not self.region:
                raise TypeError(
                    "region must be provided as keyword argument when using refreshable_credentials"
                )
            ...
            l = len(args)
            if l not in [2, 4, 5]:
                msg = f"AWS4Auth() takes 2, 4 or 5 arguments, {l} given"
                raise TypeError(msg)
    ...
    def parse_date(date_str):
        ...
        * ISO 8601 / RFC 3339 (e.g. 2009-03-25T10:11:12.13-01:00)
        ...
        formats = {
            ...
            r"^\w+day, (\d{2})-(\w{3})-(\d{2})\D.*$": lambda m: f"{str(datetime.date.today().year)[:2]}{m.group(3)}-{months.index(m.group(2).lower()) + 1:02d}-{m.group(1)}",
            # C time, e.g. 'Wed Dec 4 00:00:00 2002'
