...
class AsyncIndexTemplate:
    ...
    async def save(self, using: Any = None) -> Any:
        ...
        return await opensearch.indices.put_template(name=self._template_name, body=self.to_dict())
...
class AsyncIndex:
    ...
    def as_template(self, template_name: Any, pattern: Any = None, order: Any = None) -> Any:
        ...
        return AsyncIndexTemplate(template_name, pattern or self._name, index=self, order=order)
    ...
    def search(self, using: Any = None) -> Any:
        ...
        ``Document``\\s.
        """
        return AsyncSearch(using=using or self._using, index=self._name, doc_type=self._doc_types)
    ...
    async def save(self, using: Any = None) -> Any:
        ...
        current_settings = (await self.get_settings(using=using))[self._name]["settings"]["index"]
        if analysis:
