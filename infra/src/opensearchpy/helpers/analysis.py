...
class AnalysisBase:
    @classmethod
    def _type_shortcut(cls: Any, name_or_instance: Any, type: Any = None, **kwargs: Any) -> Any:
...
class CustomAnalysis:
    ...
    def __init__(self, filter_name: str, builtin_type: str = "custom", **kwargs: Any) -> None:
...
class CustomAnalysisDefinition(CustomAnalysis):
    def get_analysis_definition(self: Any) -> Any:
        ...
        filters = {f._name: f.get_definition() for f in self.filter if hasattr(f, "get_definition")}
        if filters:
            out["filter"] = filters
        ...
        char_filters = {
            f._name: f.get_definition() for f in self.char_filter if hasattr(f, "get_definition")
        }
        if char_filters:
        ...
        if "filters" in d:
            d["filters"] = [
                ...
                (
                    ...
                    else ", ".join(f.to_dict() if hasattr(f, "to_dict") else f for f in fs)
                )
    ...
    def get_analysis_definition(self) -> Any:
        ...
        for filters in self.filters:
            if isinstance(filters, six.string_types):
                continue
            fs.update(
                {f._name: f.get_definition() for f in filters if hasattr(f, "get_definition")}
            )
        return d
...
class ConditionalTokenFilter(CustomTokenFilter):
    ...
    def get_definition(self) -> Any:
        d = super(CustomTokenFilter, self).get_definition()
        if "filter" in d:
            d["filter"] = [f.to_dict() if hasattr(f, "to_dict") else f for f in self.filter]
        return d
    ...
    def get_analysis_definition(self) -> Any:
        ...
        return {
            "filter": {
                f._name: f.get_definition() for f in self.filter if hasattr(f, "get_definition")
            }
        }
