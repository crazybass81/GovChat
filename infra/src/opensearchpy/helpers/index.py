...
class IndexTemplate:
    def __init__(
        self, name: Any, template: Any, index: Any = None, order: Any = None, **kwargs: Any
    ) -> None:
        if index is None:
            ...
            if kwargs:
                raise ValueError(
                    "You cannot specify options for Index when passing an Index instance."
                )
    ...
    def save(self, using: Any = None) -> Any:
        ...
        return opensearch.indices.put_template(name=self._template_name, body=self.to_dict())
...
class Index:
    ...
    def as_template(self, template_name: Any, pattern: Any = None, order: Any = None) -> Any:
        # TODO: should we allow pattern to be a top-level arg?
        # or maybe have an IndexPattern that allows for it and have
        # Document._index be that?
        return IndexTemplate(template_name, pattern or self._name, index=self, order=order)
    ...
    def load_mappings(self, using: Optional[OpenSearch] = None) -> None:
        self.get_or_create_mapping().update_from_opensearch(self._name, using=using or self._using)
    ...
    def search(self, using: Optional[OpenSearch] = None) -> Search:
        ...
        ``Document``\\s.
        """
        return Search(using=using or self._using, index=self._name, doc_type=self._doc_types)
    ...
    def get_field_mapping(self, using: Optional[OpenSearch] = None, **kwargs: Any) -> Any:
        """
