...
class AttrList:
    def __init__(self, p: Any, obj_wrapper: Optional[Callable[..., Any]] = None) -> None:
    ...
    def to_dict(self) -> Any:
        ...
        for pname, value in iteritems(self._params):
            ...
            if pinfo and "type" in pinfo:
                ...
                # list of dict(name -> DslBase)
                if pinfo.get("multi") and pinfo.get("hash"):
                    value = list({k: v.to_dict() for k, v in iteritems(obj)} for obj in value)
...
class HitMeta(AttrDict):
    def __init__(self, document: Dict[str, Any], exclude: Any = ("_source", "_fields")) -> None:
        d = {
